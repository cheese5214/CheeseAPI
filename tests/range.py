...
`CheeseAPI/response.py` 的 Range 实现已修复（206、`content-range`、闭区间长度、`accept-ranges`），
本文件的断言均为正式断言（`check`）。
'''
...
def case_range_single(t, server):
    ...
    t.check('range 单区间：返回 206 Partial Content', response.status_code == 206, f'status={response.status_code}')
    t.check('range 单区间：长度为 1024 字节', len(response.content) == 1024, f'{len(response.content)} bytes')
    t.check('range 单区间：Content-Range 为 bytes 0-1023/<size>', response.headers.get('content-range') == f'bytes 0-1023/{size}', repr(response.headers.get('content-range')))
    t.check('range 单区间：返回 accept-ranges 头', response.headers.get('accept-ranges') == 'bytes', repr(response.headers.get('accept-ranges')))
...
def case_range_multi(t, server):
    ...
    t.check('range 多区间：返回 206 Partial Content', response.status_code == 206, f'status={response.status_code}')
    t.check('range 多区间：包含两段各 64 字节的内容', content[:64] in response.content and content[128:192] in response.content, f'{len(response.content)} bytes')
